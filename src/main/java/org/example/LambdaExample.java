...
public class LambdaExample {
    ...
    public static void main(final String[] args) {
        ...
        Predicate<User> sorted_by_name_userPredicate = new Predicate<User>() {
            @Override
            public boolean test(User user) {
                return user.age > 5;
            }
        };
        Comparator<User> orderByName = new Comparator<User>() {
            @Override
            public int compare(User o1, User o2) {
                return o1.name.compareToIgnoreCase(o2.name);
            }
        };
        repository.select(sorted_by_name_userPredicate, orderByName);

        banner("Listing users with age > 5 sorted by name - lambda");
        // TODO With functional interfaces used directly
        repository.select(user -> user.age > 5, (o1, o2) -> o1.name.compareToIgnoreCase(o2.name));

        banner("Listing users with age < 10 sorted by age");
        // TODO With functional interfaces declared
        Predicate<User> sorted_by_age_userPredicate = new Predicate<User>() {
            @Override
            public boolean test(User user) {
                return user.age < 10;
            }
        };
        Comparator<User> order2 = new Comparator<User>() {
            @Override
            public int compare(User o1, User o2) {
                return Integer.compare(o1.age, o2.age);
            }
        };
        repository.select(sorted_by_age_userPredicate, order2);
        ...
        // TODO With functional interfaces declared
        Predicate<User> active_sorted_by_name = new Predicate<User>() {
            @Override
            public boolean test(User user) {
                return user.active;
            }
        };
        Comparator<User> order3 = new Comparator<User>() {
            @Override
            public int compare(User o1, User o2) {
                return o1.name.compareToIgnoreCase(o2.name);
            }
        };
        repository.select(active_sorted_by_name, order3);

        banner("Listing active users sorted by name - lambda");
        // TODO With functional interfaces used directly
        repository.select(user -> user.active, (o1, o2) -> o1.name.compareToIgnoreCase(o2.name));

        banner("Listing active users with age > 8 sorted by name");
        // TODO With functional interfaces declared
        Predicate<User> sorted_by_age_userPredicate2 = new Predicate<User>() {
            @Override
            public boolean test(User user) {
                return user.age > 8;
            }
        };
        Comparator<User> order4 = new Comparator<User>() {
            @Override
            public int compare(User o1, User o2) {
                return o1.name.compareToIgnoreCase(o2.name);
            }
        };
        repository.select(sorted_by_age_userPredicate2, order4);

        banner("Listing active users with age > 8 sorted by name - lambda");
        // TODO With functional interfaces used directly
        repository.select(user -> user.age < 8, (o1, o2) -> o1.name.compareToIgnoreCase(o2.name));
